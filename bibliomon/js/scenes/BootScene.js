class BootScene extends Phaser.Scene {
    ...
    preload() {
        ...
        // ── Other assets (will be added later) ──────────────────────────────
    ...
    create() {
        ...
        // ── Player walking animations (fallback if no external sheet) ──────
        ...
        // ── Tile index map (order must match the tileChars array) ──────────
        const tileChars = [
    ...
    // ── Compose the final tileset (external or programmatic) ─────────────
    composeTileset() {
        const tileSize = 32;
        ...
        // Mapping from tileChar → file name (without extension)
        const fileNameMap = {
            '.': 'dot',   'W': 'W',     'H': 'H',     'K': 'K',
            'S': 'S',     'D': 'D',     'T': 'T',     'V': 'V',
            'C': 'C',     'E': 'E',     'L': 'L',     'B': 'B',
        ...
        const tileChars = Object.keys(fileNameMap);   // order must be consistent
        // We'll use the same order as tileChars above, but we can just iterate the array
        const orderedChars = [
            '.', 'W', 'H', 'K', 'S', 'D', 'T', 'V', 'C', 'E', 'L', 'B', 'X', 'G',
            'P>', 'P<', 'P^', 'Pv', 'Z', 'R', 'I', 'Y', 'O', 'M'
        ];
        ...
        orderedChars.forEach((char, index) => {
            ...
            if (this.textures.exists(texKey)) {
                ...
                // Fallback – draw programmatically at 32×32
